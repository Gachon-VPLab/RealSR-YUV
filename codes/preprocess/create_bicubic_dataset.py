...
if opt.dataset == 'yuv':
    path_sdsr = PATHS['datasets']['yuv'] + '/generated/sdsr/'
    path_tdsr = PATHS['datasets']['yuv'] + '/generated/tdsr/'
    input_source_dir = PATHS['yuv']['onec']['source']
    input_target_dir = PATHS['yuv']['onec']['target']
...
# generate the noisy images-> It is grayscale image, therefore it's _C = 1
with torch.no_grad():
    for file in tqdm(source_files, desc='Generating images from source'):
        # load HR image-> HR image is one channel grayscale image
...
# generate the noisy images-> It is grayscale image, therefore it's _C = 1
    for file in tqdm(target_files, desc='Generating images from target'):
        ...
        # target image from YUV is not normalised. 
        # Therefore, we should normalised to doensample images. 
        resize_img = utils.imresize_unnorm(input_img, 1.0 / opt.upscale_factor, True, 16)
