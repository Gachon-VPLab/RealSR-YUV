...
def _read_img_lmdb(env, key, size):
    ...
    # it reads just using frombuffer 
    ####################
...
def read_img(env, path, size=None):
    ...
    if env is None:  # img
        # No need to read alpha channel 
        # img = cv2.imread(path, cv2.IMREAD_UNCHANGED)
        # it automatically changed into 8-bit
        img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
    else:
        img = _read_img_lmdb(env, path, size)
        
    ####################
    # Then normalise image (standard: 8bit)
    ####################
    img = img.astype(np.float32) / 255.
    
    # shape: H, W, C
    if img.ndim == 2: # H, W (default grayscale image)
        img = np.expand_dims(img, axis=2) # H, W, C (C :1)
    # some images have 4 channels
    if img.shape[2] > 3:
        img = img[:, :, :3]
    return img # return one channel image
