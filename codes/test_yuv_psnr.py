...
for test_loader in test_loaders:
    ...
    for data in test_loader:
        ...
        if need_GT:
            ...
            print(f'shape of gt_img: {gt_img.shape}')

            if gt_img.shape[2] == 1:  # RGB image -> YCbCr image
                sr_img_y = sr_img # bgr2ycbcr(sr_img, only_y=True)
                gt_img_y = gt_img # bgr2ycbcr(gt_img, only_y=True)
                if crop_border == 0:
                ...
                logger.info(
                    'C: 1 | {:20s} - PSNR: {:.6f} dB; SSIM: {:.6f}; PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}.'.
